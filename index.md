...
### 2-1. 사진과 사진에 담긴 위치 정보

| 항목 | 내용 |
|---|---|
| 무엇 | 사용자가 직접 고른 사진, 그 사진의 촬영 시각과 GPS 좌표(EXIF) |
| 왜 | 후기 초안을 만들기 위해. 좌표는 어디에서 찍었는지 알아내 글에 넣기 위해 |
| 어디로 | 사진은 Google Gemini API로, 좌표는 지명 변환 서비스로 전송 |
| 얼마나 | 앱은 전송한 사진을 서버에 보관하지 않음 |

사진 접근 권한과 위치 정보 접근 권한(`ACCESS_MEDIA_LOCATION`)은 **거부해도 앱을 쓸 수 있습니다.** 거부하면 사진을 직접 고르는 방식으로만 동작하고, 장소는 사용자가 직접 입력하게 됩니다.
...
사진에 좌표가 있을 때만, 그 좌표만 보냅니다. 사진이나 후기 내용은 보내지 않습니다.
