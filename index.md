...
**시행일: 2026년 9월 17일** (아이폰판 출시에 맞춰 iOS 내용을 더했습니다)
...
**사진을 고를 때는 권한을 요청하지 않습니다.** 안드로이드와 아이폰 모두 시스템 사진
선택 화면을 쓰기 때문에, 사용자가 그 화면에서 고른 사진만 앱으로 넘어옵니다. 나머지
사진은 앱이 볼 수 없습니다.

**아이폰에서는 한 가지 예외가 있습니다.** 앱에서 카메라로 찍은 사진을 사진 앱의
"리도" 앨범에 저장할 때 사진 보관함 권한을 요청합니다. 이 권한은 그 앨범에 저장하는
데만 쓰며, 보관함의 다른 사진을 읽거나 밖으로 보내지 않습니다. 앱 설정에서
"찍은 사진 갤러리에 저장"을 끄면 요청하지 않습니다.

안드로이드의 선택 도구는 사진의 위치 태그를 지우고 넘겨주는 것이 기본이고, 아이폰은
사진에 따라 위치 태그가 함께 넘어올 수 있습니다. 앱은 대개 사진의 좌표를 쓰지 않고,
장소는 사용자가 직접 적거나 지도 검색으로 고릅니다. 좌표가 남아 있는 사진이라면 그
좌표만 지명 변환 서비스로 보냅니다.
...
- **안드로이드**: AdMob은 광고 식별자(AAID) 등을 이용해 맞춤 광고를 제공할 수 있습니다.
- **아이폰**: 앱은 "앱 추적 허용"을 요청하지 않습니다. 그래서 AdMob은 광고 식별자(IDFA)를
  받지 못하고, 맞춤 광고가 아닌 광고를 보여 줍니다. 다만 광고를 내보내고 측정하기 위해
  앱 단위 기기 식별자, IP 주소로 짐작한 대략적인 지역, 광고 노출·클릭, 오류 정보를 수집할
  수 있습니다.
...
- 안드로이드: 맞춤 광고는 **기기 설정 → Google → 광고**에서 끌 수 있습니다.
- 아이폰: **설정 → 개인정보 보호 및 보안 → 추적**에서 앱의 추적 요청을 막을 수 있습니다. 리도는 추적을 요청하지 않습니다.
...
**보내지 않는 것**: 사진, 만들어진 후기 글, 입력한 내용, 이름·이메일·전화번호, 광고 ID, 기기 식별자(ANDROID_ID, IDFA, IDFV 등).
...
- **사진 접근**: 사진을 고를 때는 권한이 없어 철회할 것도 없습니다.
  사진은 고를 때마다 시스템 선택 화면에서 그때 고른 것만 넘어갑니다.
  아이폰에서 찍은 사진 저장용 권한은 **설정 → 리도 → 사진**에서 끌 수 있습니다
- **맞춤 광고 끄기**: 안드로이드 설정 → Google → 광고 (아이폰은 추적을 요청하지 않아 맞춤 광고가 나가지 않습니다)
...
**Effective: September 17, 2026** (iOS details added for the iPhone release)
...
**Picking photos needs no permission.** On both Android and iPhone the app uses the system
photo picker, so only the photos you select on that screen come through. The app cannot see
the rest.

**There is one exception on iPhone.** When you take a photo with the camera in the app, saving
it to a "Rido" album in the Photos app asks for photo library permission. That permission is
used only to save to that album. The app does not read or send any other photo in your
library. Turning off "Save photos to gallery" in the app's settings means it is never asked.

Android's picker strips location tags by default; on iPhone a photo may still carry them. The
app usually does not use a photo's coordinates, and you either type the place yourself or search
the map. If a photo does carry coordinates, only those coordinates go to a place-name service.
...
- **Android**: AdMob may use the advertising identifier (AAID) and similar signals to serve
  personalized ads.
- **iPhone**: the app does not ask for permission to track. AdMob therefore does not get the
  advertising identifier (IDFA) and shows non-personalized ads. To deliver and measure ads it
  may still collect an app-scoped device identifier, an approximate region inferred from your IP
  address, ad impressions and taps, and error information.
...
- Android: personalized ads can be turned off in **device Settings → Google → Ads**.
- iPhone: you can block apps from asking to track in **Settings → Privacy & Security → Tracking**. Rido does not ask to track.
...
**Not sent**: photos, generated reviews, anything you typed, your name, email, phone number, advertising ID, or device identifiers such as ANDROID_ID, IDFA, or IDFV.
...
- **Photo access**: picking photos needs no permission, so there is nothing to revoke.
  Each time, only what you pick on the system picker is handed over.
  On iPhone, the permission used to save camera photos can be turned off in
  **Settings → Rido → Photos**
- **Turn off personalized ads**: Android Settings → Google → Ads (on iPhone the app does not ask to track, so ads are not personalized)
